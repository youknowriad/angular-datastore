        ...
        function ($rootScope, $q, AngularDataModelFactory, AngularDataSerializer, AngularDataRestAdapter) {
            ...
            hydrateOrLoad = function (record) {
                ...
                if (angular.isDefined(existingRecord)) {
                    AngularDataSerializer.hydrate(existingRecord, AngularDataSerializer.serialize(record, false), store);
            ...
            handleSocketIO = function (type) {
                socket.on(type + '.update', function(data) {
                    $rootScope.$apply(function () {
                        var record = AngularDataSerializer.unserialize(type, data, store);
                        hydrateOrLoad(record);
                    });
                });
